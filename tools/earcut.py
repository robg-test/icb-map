"""Pure-python port of mapbox/earcut (ISC).

Only the parts we need: simple polygons plus optional holes, 2D, returns a flat
list of vertex indices (triples). No third-party deps -- the box this runs on has
a bare python3.

Input is a flat coordinate array [x0, y0, x1, y1, ...] like the JS original, so
the index arithmetic below matches the reference implementation one-to-one.
"""
...
def earcut(data, hole_indices=None, dim=2):
    ...
    # bounding box, only worth it (and only correct) for larger rings
...
def _earcut_linked(ear, triangles, dim, min_x, min_y, inv_size, pass_):
    ...
    while ear.prev is not ear.next:
        ...
        if ear is stop:
            # no ear found -- try the progressively more expensive fallbacks
            if pass_ == 0:
                _earcut_linked(
...
def _find_hole_bridge(hole, outer_node):
    ...
    # find the segment to the left of the hole's leftmost point
    while True:
    ...
    # look for a reflex vertex inside the hole-bridge triangle, take the closest
    stop = m
    mx = m.x
    my = m.y
    tan_min = float("inf")
    ...
    while True:
        if (
            ...
            if _locally_inside(p, hole) and (
                ...
                or (
                    tan == tan_min
                    and (p.x > m.x or (p.x == m.x and _sector_contains_sector(m, p)))
                )
            ):
                m = p
                tan_min = tan
...
def _z_order(x, y, min_x, min_y, inv_size):
    x = int((x - min_x) * inv_size) | 0
    y = int((y - min_y) * inv_size) | 0
...
def _middle_inside(a, b):
    ...
    while True:
        if ((p.y > py) != (p.next.y > py)) and p.next.y != p.y:
            if px < (p.next.x - p.x) * (py - p.y) / (p.next.y - p.y) + p.x:
                inside = not inside
        p = p.next
        if p is a:
            break
